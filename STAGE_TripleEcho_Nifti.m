...
%dirA1='2_STAGE_FA10_1.1x1.1x4.0mm_Grappa2\';
%dirA1p='09-Unknown\';

%dirA2='02-A2-T1W-mag\';
%dirA2p='03-A2-T1W-pha\';
%dirA2='3_STAGE_FA50_1.1x1.1x4.0mm_Grappa2\';
%dirA2p='05-Unknown\';
...
bSaveDicom = 0; % 1: to save dicom format; otherwise not to save dicom
...
ddx = info.PixelDimensions(1);
ddy = info.PixelDimensions(2);
ddz = info.PixelDimensions(3);

imsize = size(timage);
numfiles = length(nifti_files);


magA1 = zeros(imsize(1), imsize(2), imsize(3), numfiles/2);
magA2 = zeros(imsize(1), imsize(2), imsize(3), numfiles/2);
All_im = zeros(imsize(1), imsize(2), imsize(3), numfiles);
All_im_sep = zeros(imsize(1), imsize(2), imsize(3), numfiles/2,2);

te_buffer = zeros(1,numfiles);
fa_buffer = zeros(1,numfiles);

for i=1:numfiles
    fname = strcat(selpath, '\',nifti_files(i).name);

    te_buffer(i) = extractTE_STAGE(fname);
    fa_buffer(i) = extractFA_STAGE(fname);
...
te_buffer
fa_buffer
all_buffer = [te_buffer;fa_buffer]
sall_buffer = sum(all_buffer,1)

ute_buffer = unique(te_buffer)
ufa_buffer = unique(fa_buffer)

nte = length(ute_buffer)
nfa = length(ufa_buffer)


for i = 1:nfa
    for j = 1:nte
    d = find(sall_buffer == ute_buffer(j) + ufa_buffer(i))
    All_im_sep(:,:,:,j,i) = All_im(:,:,:,d);
